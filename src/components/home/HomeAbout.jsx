...
const HomeAbout = () => {
  ...
  return (
    <section id="about" className={theme === "light" ? "" : "bg-primaryDark"}>
      <Container
        className={`py-20 xl:py-30 flex flex-col lg:flex-row items-center gap-13`}
      >
        <div className="image w-[90%] sm:w-3/4 lg:w-1/2  aspect-square rounded-full bg-primaryText relative z-10">
          ...
          <div className="grid grid-cols-5 gap-3 sm:gap-6 absolute -z-10 top-0 left-0">
            {Array.from({ length: 30 }).map((_, i) => (
              <div
                key={i}
                className="w-1 sm:w-2 aspect-square bg-primary rounded-full"
              />
            ))}
          </div>
          <img
            className="absolute bottom-0 right-0 w-26 sm:w-auto"
            src={element}
            alt=""
          />
        </div>
